...
import { getUserWishlist, rmoveWishListItem } from '@/lib/servises/Wish';
...
export default function WishCard({ data, handleRemoved }: { data: IProduct; handleRemoved: (id: string) => void }) {
...
async function remove() {
    await rmoveWishListItem(data._id);
    handleRemoved(data._id);
  ...
  <div className="card-body" >
   <div onClick={()=>{handleNavigate(id)} }> 
    ...
    <div className="card-actions justify-end">
      <div className="bg-transparent w-full text-green-500">{data.brand?.name}</div>

      <div className=' mr-auto'>{data?.price} EGP</div>
      <div className="flex items-center space-x-1 rtl:space-x-reverse">
  <span className="text-sm font-medium">{data?.ratingsAverage}</span>
  <svg 
...
<button onClick={() => remove()} className="w-10 h-10 btn bg-red-400 rounded-full p-0 hover:bg-red-600 hover:scale-110 transition duration-300  ">
